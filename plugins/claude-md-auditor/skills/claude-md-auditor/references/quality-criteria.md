...
## Bonus: Modern Features Usage

Not all projects need these, so don't penalize their absence. But when a project would benefit, flag the opportunity:

- **`.claude/rules/`** — is the CLAUDE.md over 200 lines with multi-domain content? Suggest splitting
- **Path-specific rules** — are there instructions that only apply to certain file types? Suggest scoping with `paths:` frontmatter
- **`@imports`** — is content duplicated from README or other docs? Suggest importing instead
- **`claudeMdExcludes`** — in a monorepo, are irrelevant CLAUDE.md files loading? Suggest excluding
...
- CLAUDE.md over 300 lines (context saturation — Claude stops following instructions)
- Style rules that a linter should enforce (ESLint, Prettier, Black, etc.)
- Embedded code snippets that may have drifted from reality (use `file:line` references)
- Instructions that should be hooks ("always run X before committing")
- Infrequent workflow descriptions that should be skills
